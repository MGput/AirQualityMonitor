...
void CurlServiceFundaments::initializeCurl() {
    ...
    if (!curl) cerr << "B≥πd inicjalizacji CURL" << endl;
}
...
json CurlServiceFundaments::makeRequest(const string& url) {
    ...
    if (curl) {
        ...
        curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, WriteCallback);               // Ustawienie funkcji i wskaünika do odbierania danych
        ...
        if (reqResult != CURLE_OK) {
            cerr << "B≥πd zapytania: " << curl_easy_strerror(reqResult) << endl;
        }
        else {
            long HTTPstatus = 0;
            ...
            if (HTTPstatus == 200) {                                                // Kod OK
                ...
                catch (const json::parse_error& e) {                                // Obs≥uga b≥ÍdÛw JSON
                    cerr << "B≥πd parsowania JSON: " << e.what() << endl;
                }
            }
            else {                                                                  // Obs≥uga b≥ÍdÛw HTTP
                cerr << "B≥πd HTTP " << HTTPstatus << endl;
            }
        }
    }
